    ...
    try {
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user",
            content: `اجلب بيانات الموظفين من هذا الرابط وأرجع JSON فقط بدون أي نص إضافي:
GET ${GS_URL}?action=getEmployees

أرجع فقط محتوى JSON المُرجَع من الرابط كما هو بدون تعديل.`
          }],
          tools: [{
      ...
      console.warn("gsGetEmployees via API failed", e);
      resolve([]);
    }
  });
}
